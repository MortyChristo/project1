...
def test_add_re_amount_negative(mocker):
    ...
    with pytest.raises(ReimbursementError) as excinfo:
        employee_serv.add_reimbursement(reimbursement_obj)

    assert str(excinfo.value) == "Amount needs to be positive"


def test_add_re_positive(mocker):
    reimbursement_obj = Reimbursements(100001, 200, "pending", "a", "n/a", 1)
    def mock_add_reimbursement(self, re_obj):
        if re_obj.amount is not None:
            return reimbursement_obj
    mocker.patch('backend.dao.reimbursement_dao.ReimbursementDao.add_reimbursement', mock_add_reimbursement)
    re_service = er.ReimbursementService()
    actual = re_service.add_reimbursement(reimbursement_obj)
    assert actual == {
        'amount': 200,
        'description': 'n/a',
        'employee_id': 100001,
...
def test_view_all_positive(mocker):
    dict_re = []
    def mock_view(self):
        reimbursement_obj1 = Reimbursements(100001, 200, "pending", "a", "n/a", 1)
        reimbursement_obj2 = Reimbursements(100001, 200, "pending", "a", "n/a", 1)
        reimbursement_obj3 = Reimbursements(100002, 200, "pending", "a", "n/a", 1)
        dict_re.append(reimbursement_obj1)
        dict_re.append(reimbursement_obj2)
        dict_re.append(reimbursement_obj3)
        return dict_re
    mocker.patch('backend.dao.reimbursement_dao.ReimbursementDao.view_all_reimbursements', mock_view)
    re_service = er.ReimbursementService()
    actual = re_service.view_all_reimbursements()
    assert actual == [{
        'amount': 200,
        'description': 'n/a',
        'employee_id': 100001,
...
def test_view_by_status_positive(mocker):
    dict_re = []
...
def test_change_a(mocker):
    reimbursement_obj1 = Reimbursements(100001, 200, "pending", "a", "n/a", 1)
    def mock_change(re_id):
        if re_id == 1:
            reimbursement_obj1.status = "Approved"
    return reimbursement_obj1

    mocker.patch('backend.dao.reimbursement_dao.ReimbursementDao.change_status_a', mock_change)
    re_serv = er.ReimbursementService()
